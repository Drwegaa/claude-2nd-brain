...
4. **Push to GitHub** — Run: `cd {{VAULT_PATH}} && git add -A && git diff --staged --quiet || git commit -m "vault-sync: $(date '+%Y-%m-%d %H:%M')" && git push origin main`

5. **Confirmation** — End with: "Vault sync complete. [X] inbox notes filed, [Y] personal notes updated, [Z] broken links flagged. Pushed to GitHub."
