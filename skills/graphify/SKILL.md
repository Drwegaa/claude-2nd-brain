...
Replace PATH_TO_DIR with the actual absolute path.
...
Before running any subcommand, check that `.graphify_python` exists:
...
If new files exist, check whether all changed files are code files. If code-only: skip semantic extraction, run only AST. Then merge and run Steps 4-8.
...
for neighbor in G.neighbors(nid):
    ...
    print(f'  --{edge.get(\"relation\",\"\")}--  {G.nodes[neighbor].get(\"label\", neighbor)} [{edge.get(\"confidence\",\"\")}]')
"
```
...
After successful save, run `--update` pipeline on `./raw`.
